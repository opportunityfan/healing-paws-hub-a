...
function postArchiveDetailInfo(){
  if (archiveDetailInfo.name === '') {
    showMessage("病例名称不能为空","warning")
    return
  }
  else if (archiveDetailInfo.type == "") {
    showMessage("所患疾病不能为空","warning")
    return
  }
  else {
...
let alertDeleteArchive = ref(false)

function deleteArchive() {
  axios.delete("/case",{
    params:{
      id: archiveDetailInfo.id
    },
    headers:{
      'token':store.state.token
    }
      ...
      <HScroller scroll-direction="column" class="full scroller-view">
        <div class="flex-column">
          ...
          <div class="archive-content">
            ...
            ></HInput>
            <div class="archive-title">所患疾病</div>
            <span>按住shift或ctrl多选：</span>
            ...
            <br><br>
            <div class="flex-row">
              <HButton @click="postArchiveDetailInfo" style="margin-right: 10px">
                <span v-if="archiveDetailInfo.id!==''">确认修改</span>
                <span v-else>新建病例</span>
              </HButton>
              <HButton v-show="archiveDetailInfo.id!==''" @click="alertDeleteArchive=true">删除病例</HButton>
            </div>
          </div>
        </div>
      </HScroller>
    </div>
    <HAlert v-model="alertDeleteArchive">
      <div class="flex-column" style="gap: 10px; text-align: left">
        ...
        <div class="flex-row" style=" width: 100%;gap: 10px; justify-content: flex-end">
          <h-button type="secondary" height="30px" style="margin: 0; width: 60px; font-size: 12px" id="cancel" @click="alertDeleteArchive=false">取消</h-button>
          <h-button type="danger" height="30px" style="margin: 0; width: 60px; font-size: 12px" @click="deleteArchive" id="confirm">删除</h-button>
        </div>
      </div>
    </HAlert>
  </HLoading>
